...
class AttentionHead(nn.Module):
    ...
    def forward(self, x, mask = None):
        ...
        qkv = self.to_query_key_value(x) #gets q = Q = Wq matmul x1, k = Wk mm x2, v = Wv mm x3
...
class Transformer(nn.Module):
    ...
    def forward(self, x, mask = None):
        for attention, mlp in self.layers:
            ...
            x = mlp(x) #go to MultiLayerPercep
        return x

class ImageTransformer(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, att_heads, mlp_dim, channels = 3, dropout = 0.1, emb_dropout = 0.1):
        super().__init__()
